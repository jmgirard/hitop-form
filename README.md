...
| `tests/layout.spec.js` | On every page of the HiTOP-SR and the PID-5, at 320 px, 375 px and the default width, each item's text box lies inside its card's border on all four sides and does not overflow, with at least one wrapped item measured. Each item on a first page is a group named by its position and text. A refused blank item's card has the error colour on all four borders |
